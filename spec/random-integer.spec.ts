...
describe(`iterative`,function() {
    ...
    for(let i = min; i <= max; i++) {
        ...
        describe(`inclusive`,function() {
            ...
            it(`result ${min - 1} - ${i}`, () => {
                {
                    const random = RandomIntegerParameters(min - 1, i);
                    expect(random).toBeLessThanOrEqual(i);
                    expect(random).toBeGreaterThanOrEqual(min - 1);
                }

                {
                    const random = RandomIntegerParameter({minimum:min - 1, maximum:i, inclusive : true});
                    expect(random).toBeLessThanOrEqual(i);
                    expect(random).toBeGreaterThanOrEqual(min - 1);
                }
            ...
            it(`result ${min + max} - ${i+max}`, () => {
                {
                    const random = RandomIntegerParameters(min + max, i+max);
                    expect(random).toBeLessThanOrEqual(i + max);
                    expect(random).toBeGreaterThanOrEqual(min + max);
                }

                {
                    const random = RandomIntegerParameter({minimum:min+max, maximum: i + max});
                    expect(random).toBeLessThanOrEqual(i + max);
                    expect(random).toBeGreaterThanOrEqual(min + max);
                }
            });
        ...
        describe(`exclusive`,function() {
            ...
            it(`result ${min} - ${val1}`, () => {
                {
                    const random = RandomIntegerParameters(min, val1, false);
                    expect(random).toBeLessThan(val1);
                    expect(random).toBeGreaterThan(min);
                }

                {
                    const random = RandomIntegerParameter({minimum: min, maximum: val1, inclusive: false});
                    expect(random).toBeLessThan(val1);
                    expect(random).toBeGreaterThan(min);
                }
            });
            ...
            it(`result ${min} - ${val2}`, () => {
                {
                    const random = RandomIntegerParameters(min, val2, false);
                    expect(random).toBeLessThan(val2);
                    expect(random).toBeGreaterThan(min);
                }

                {
                    const random = RandomIntegerParameter({minimum: min, maximum: val2, inclusive: false});
                    expect(random).toBeLessThan(val2);
                    expect(random).toBeGreaterThan(min);
                }
            });
...
describe(`minimum & maximum equal`,function() {
    ...
    it(`inclusive`,function() {
        ...
        {
            const random = RandomIntegerParameters(1, 1, true);
            expect(random).toBe(1);
        }
        {
            const random = RandomIntegerParameter({minimum: 1, maximum: 1, inclusive: true});
            expect(random).toBe(1);
        }
